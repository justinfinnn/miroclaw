<template>
  <div class="process-page">
    <!-- Top navigation bar -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">MIROFISH LOCAL</div>
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Automatically start graph building (only if no build task already exists)
      if (response.data.status === 'ontology_generated' && !response.data.graph_id && !response.data.graph_build_task_id) {
        await startBuildGraph()
      }

      // Continue polling running build tasks
      if ((response.data.status === 'ontology_generated' || response.data.status === 'graph_building') && response.data.graph_build_task_id) {
